...
<h1 align="center" style="font-family:Franklin Gothic Bold"> Laporan Data BP </h1>
<p>
	<?php include 'lap_menu.php';  ?>
  <a href="preview_bp.php"><img src="dist/img/print.jpg" width="40"> PREVIEW</a>
</p>
...
<?php
	...
	if ($_POST['btnCari'] == "Cari") {
		$sql = mysql_query("SELECT p.*,i.nm_tersangka FROM tb_bp p LEFT JOIN tb_spdp s ON(s.id_spdp=p.id_spdp) 
			left join  tb_identitas i ON(s.id_identitas=i.id_identitas) WHERE
      id_spdp LIKE '%".$_POST['txtCari']."%' 
      ORDER BY id_bp ") or die (mysql_error());
	} else {
		$sql = mysql_query ("SELECT p.*,i.nm_tersangka FROM tb_bp p LEFT JOIN tb_spdp s ON(s.id_spdp=p.id_spdp) left join tb_identitas i 
			ON(s.id_identitas=i.id_identitas) ORDER BY id_bp LIMIT $posisi , $batas ") or die (mysql_error());
	}
	$no=$posisi+1;
...
<tr>
	<td><?php echo $no; ?></td>
	<td><?php echo $isi['id_bp'];?></td>
	<td><?php echo $isi['nm_tersangka'];?></td>
	<td><?php echo $isi['tgl_bp'];?></td>
	...
	$sql2 = mysql_query ("SELECT * FROM tb_bp") or die (mysql_error());
